...
*   Using a terminal emulator, send an AT command to ensure that the smart terminal can receive AT commands. For more information, see [General AT commands](../at-command-reference/general-at-commands/).
...
*   Send an AT+ETMINFO=version command to verify that the version number is AnyNet SMARTconnect™ V0.99\_ma or higher.
    ...
    If the version number is not in this range, you will need to update the modem software. For more information, see [Over-the-air (OTA) updates](../updates/over-the-air-ota-updates.md).
...
2. Subscribe to the newly created publish topics using your cloud provider's console.
   ...
   2.  In the **Topic filter** box, enter:
...
3.  Publish information to the topics you created in the module.
    ...
    1.  Using the terminal emulator, enter:
