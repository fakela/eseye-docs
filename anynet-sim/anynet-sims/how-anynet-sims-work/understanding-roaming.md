...
* Limits to how long a device can roam in a country before the subscriber must switch to a local operator, depending on [data sovereignty and network policies](understanding-roaming.md#data-sovereignty-and-network-policies).
* Increased costs, for example a higher rate for voice, data or SMS services.
* Reduced services, for example no support for [power saving features](iot-wireless-technologies.md) when roaming.
* [Latency](understanding-roaming.md#latency) and performance issues because data is backhauled to the home operator's core network.

Customer preferences for [global data distribution](understanding-roaming.md#customer-preferences-for-global-data-distribution) may also limit the available roaming networks.

To avoid these limitations, you can [localise the SIM](understanding-localisation.md), or [steer the SIM](understanding-roaming.md#steering-of-roaming) onto a roaming partner with a more favourable roaming agreement.
...
Latency when roaming is typically higher than on a local profile. For more information, see [Measuring latency](../../connectivity/monitoring-and-diagnostics/measuring-latency.md).
...
GDPR is also a global requirement.
