...
The embedded Universal Integrated Circuit Card (eUICC) is the eSIM software component that runs on a UICC. It is responsible for the storage of multiple [network profiles](about-euicc-sim-profiles.md) and the secure, over-the-air (OTA) [remote SIM provisioning](understanding-the-gsma-rsp-m2m-general-architecture.md) and management of profiles.

Each eUICC SIM has a unique identifier ([EID](https://app.gitbook.com/s/hPiB0Z0YdVhl1D9teuS7/#eid)) and uses the [ICCID and IMSI](euicc-overview.md#iccid-and-imsi) values from the enabled profile.

![](../../.gitbook/assets/eSIM_eUICC.svg)
...
For eUICC SIMs, the SIM supplier identifier is the initial digits of the EID. For Eseye eUICC SIMs, the identifier is either:
...
1.  Open a logical channel to the eUICC SIM. Type:
    ...
    where `<nn>` is the allocated channel number, for example, `01`, and `9000` is a successful command. This is only valid for channels 1-7 (depending on the SIM type).
2.  Select the eUICC Controlling Authority Security Domain (ECASD) using the Application ID (AID) **A0000005591010FFFFFFFF8900000200**. Type:
    ...
    where `<nn>` is the allocated channel number, for example, `01`:
...
3.  Read data from the ECASD. Type:
    ...
    ```
    AT+CSIM=10,"8<nn>CA005A12"
    ```

    where `<nn>` is the channel number with the most significant bit set, in hexadecimal. For example, `81CA005A12` is for channel 1, and `82CA005A12` is for channel 2.
...
On eUICCs, the ICCID and IMSI are unique to a profile and not the eUICC, which is uniquely identified using the EID.

![](../../.gitbook/assets/eSIM_EuiccIccidAndImsi.svg)
